    ...
    <div className={`min-h-screen p-6 ${isDark ? 'bg-black text-white' : 'bg-gray-50 text-gray-900'}`}>
      <div className="max-w-4xl mx-auto">
        ...
        {loading ? (
          ...
          <p className={isDark ? 'text-white/40' : 'text-gray-400'}>No bug reports found.</p>
        ) : (
              ...
              >
                  ...
                  <div className="flex gap-2 mt-3">
                    {report.imageUrls.split('|||').map((url, i) => (
                      <a key={i} href={url} target="_blank" rel="noopener noreferrer">
                        <img src={url} className="w-24 h-24 object-cover rounded-lg border border-white/10" />
                      </a>
                    ))}
                  </div>
                )}
                ...
                {expandedId === report.id && (
                  <div className={`mt-3 pt-3 border-t text-xs space-y-1 ${
                    ...
                    {report.page && <p>Page: {report.page}</p>}
                    {report.userAgent && <p className="truncate">UA: {report.userAgent}</p>}
