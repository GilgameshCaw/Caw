...
const WelcomePage: React.FC = () => {
  ...
  const [loadingMessage, setLoadingMessage] = useState(t('welcome_page.loading_profile'))
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        if (tokenId) {
          try {
            ...
            if (totalElapsed >= TOTAL_TIMEOUT) {
              if (!cancelled) {
                setError(t('welcome_page.error.timeout'))
                setLoading(false)
              }
              return
            }
        ...
        setLoadingMessage(t('welcome_page.loading_profile'))
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          <div className="max-w-md mx-auto px-6 text-center">
            <div className={`rounded-lg p-6 mb-4 ${isDark ? 'bg-red-900/20 border border-red-500/30' : 'bg-red-50 border border-red-300'}`}>
              ...
              <h2 className={`text-xl font-bold mb-2 ${isDark ? 'text-white' : 'text-black'}`}>{t('welcome_page.error.title')}</h2>
      ...
      <>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          <div className={`text-center px-6 py-8 rounded-2xl backdrop-blur-sm ${isDark ? 'bg-black/40' : 'bg-white/40'}`}>
            ...
            <p className={`text-xs mt-2 ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>{t('welcome_page.may_take_minute')}</p>
          </div>
        </div>
        <div className="fixed bottom-5 left-5 md:right-5 md:left-auto z-[101]">
          <button
