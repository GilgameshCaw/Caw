...
- **No KYC.** No accounts beyond an NFT held in a wallet. (Optional sponsor-side withdraw gates exist for jurisdictionally-constrained fiat-mint flows — see [§6.11](#611-sponsored-mint-withdraw-gates) — but every self-funded and default-sponsored mint is unconditionally ungated.)
...
### Self-custodial fund moves without ETH

A passkey-backed user who has never held ETH still needs an unconditional path to move their own funds — withdraw their CAW back to L1 (to sell it on their own, at any external address they choose) or zap more ETH into their stake. `SmartEOA` exposes `executeBatch(Call[] calls, uint256 nonce, bytes sig)` for exactly this. A single passkey (or `ecdsaFallback`) signature authorizes an ordered batch of calls; the signed digest binds **every** call's target, value, and calldata, plus a dedicated monotonic `executeNonce` and the account+chain identity. Because authorization rides entirely in the signature, *anyone* may submit the transaction — the user, a relayer, or the Network's validator paying the gas — and the submitter can run only exactly what was signed. They cannot add, drop, reorder, or redirect a call. The withdrawal recipient is part of the signed batch, so a relayer can neither steal nor reroute the funds; the nonce is consumed before the external calls (checks-effects-interactions), so a batch cannot be replayed.

This makes a gas-free withdrawal model possible: a user signs a batch that withdraws their CAW *and* pays the relaying validator a CAW fee covering the gas the validator fronts, so relaying is economically sustainable rather than a subsidy. The fee and the recipient are both signed calls, so the relayer can neither inflate the fee nor divert the principal. The relay endpoint that submits these batches is the policy gate: it admits only batches whose targets are known CAW contracts and whose attached ETH value is bounded, since `executeBatch` is intentionally general while the off-chain relay is where the operator's wallet is protected. Unlike the sponsored mint/deposit/authenticate entry points — which exist for the cold-start case and are sponsor-submitted — `executeBatch` is the user's own authority over their own funds, and `withdrawTo` remains, as ever, never something a sponsor can trigger on a schedule the user does not control.
...
## 6.11 ==Sponsored-mint== withdraw gates

[§6.6](#66-sponsored-entry-points-and-the-three-signing-populations) covers the *mechanism* by which a sponsor server submits a mint or deposit transaction on a user's behalf. This section covers what a sponsor can *optionally* attach to that mint to constrain the resulting profile's withdraw behavior, and why those optional constraints exist.

There are three independent withdraw-side fields a sponsor can supply at `mintAndDepositSponsored` time:

1. **`kycLevel`** — chooses one of three withdraw gates (no gate, time-lock, KYC verifier).
2. **`sponsorTokenId` + `repayAmount`** — declares an L2-side repay obligation that auto-sweeps from withdrawals back to the sponsor.

These fields are **orthogonal**. Any subset can be active. None of them affect *posting, staking, or any other social action* — they apply only at the withdraw boundary.

### `kycLevel` — the withdraw gate

The sponsor picks one of these values at mint time. The level is stored per-tokenId on `CawProfileMinter` and read by `CawProfile.withdrawTo` via `checkWithdrawAllowed`:

| Level | Gate at withdraw | Who chooses this |
|---|---|---|
| **0** | None | Default for any sponsored mint — gifts, repay-only sponsorships, casual sponsorships. Identical withdraw behavior to a self-funded mint. |
| **1** | 180-day time-lock, no KYC ever | Sponsors offering fiat-card mints, where the deposit needs to look like platform stored-value rather than a fiat-to-crypto conversion. |
| **2+** | KYC verifier required at that level | Sponsors operating in jurisdictions where a specific verification level is required by regulators. Verifier slots (Civic Pass, zkMe, etc.) are added via `PathwayExpander.addKycVerifier`. |

Levels 1 and ≥2 are **mutually exclusive paths**. A level-1 token cannot be unlocked early by KYC; a level-≥2 token cannot be waited out. The sponsor chooses which gate (if any) at mint time and the user lives with that choice for the life of the profile — or until `unlockWithdraw` clears it under the matching condition.

**Self-funded mints (Population A, no sponsor) are unconditionally gate-free.** They never write the per-tokenId `mintedAt` slot, so `checkWithdrawAllowed` returns early before reading any level. The manifesto rule "No KYC" applies to the *default protocol path* — anyone who funds their own profile mint with their own CAW is never gated.

### Why the optional gates exist

This is the question worth being direct about. CAW's philosophical position is no-KYC, but a *protocol-level* prohibition on the existence of KYC-gate slots would block any sponsor who needs them for a specific user-facing flow. Two such flows are foreseeable:

- **Fiat-card sponsored mints**, where a user pays in dollars via Stripe / Apple Pay / Google Pay and the sponsor server burns its own CAW to mint a profile. Without a withdraw gate of *some* kind, the entire flow looks (under US and EU regulatory frameworks) like a fiat-to-crypto on-ramp, which is heavily regulated. The 180-day time-lock at level 1 lets the sponsor make the "stored-value" argument: the user spent dollars to use the social platform, the CAW deposit is platform credit, and it cannot be quickly liquidated to free-floating crypto. This is the same regulatory framing used by gift cards, Steam wallet credits, etc.
- **Jurisdiction-specific sponsored mints**, where a sponsor operates inside a region whose regulator requires a specific KYC level (e.g. EU MiCA tiers). Levels 2-4 give that sponsor a slot to wire a verifier *without* needing a protocol redeploy. The verifier address is itself additions-only via `PathwayExpander` — see [§11.2](#112-the-pathwayexpander-mechanic).

Importantly, none of this changes what *the protocol itself does*. The contract has a per-tokenId state slot and a withdraw-time check. Whether anyone ever passes a non-zero level depends entirely on sponsor server operators making that choice for the users they sponsor. A user who wants no gate can either self-fund or use a sponsor that doesn't set one. The manifesto's "No KYC" rule survives because the default — the path every user gets unless a sponsor consciously overrides it — is `kycLevel = 0`, no gate at all.

### Repay obligation (orthogonal to the gate)

`sponsorTokenId` and `repayAmount` are wholly separate from `kycLevel`. They declare that some portion of any withdrawal from this profile should sweep back to the sponsor's profile on L2 (credited to `cawOwnership[sponsorTokenId]`) until the obligation is satisfied. The cap is enforced at L1 mint time as `repayAmount ≤ depositAmount × 2`. The sponsor can clear the obligation at any time via `forgiveSponsorRepay` on L2 (proving ownership of `sponsorTokenId`).

A "repay-only" sponsor sets `kycLevel = 0` (no withdraw gate) and `repayAmount > 0` (auto-sweep on first withdrawals). This is the common case for the planned fiat-onramp: the sponsor advances CAW for the user, and recovers it from the user's first withdrawals; the user is otherwise unrestricted and never KYC'd.

A repay obligation does **not** activate the time-lock. Setting `repayAmount > 0` with `kycLevel = 0` leaves the profile completely ungated for posting, staking, and L2 actions; the only effect is on withdraw routing. The contract explicitly skips writing `mintedAt` when `kycLevel == 0` so a repay-only mint cannot accidentally activate the level-1 time-lock check.

### Manifesto consistency

The default sponsored mint is "no gate, no lock, no KYC" — same as the manifesto promises. The protocol *also* exposes optional fields a sponsor can use to add constraints to mints *the sponsor pays for*, in *jurisdictions or business models the sponsor operates in*. The community at large is never gated; only the specific users who consent to a constrained sponsor flow (typically by clicking "Pay with card" instead of "Connect wallet") are.
...
The complete CAW Manifesto is reproduced **verbatim** below from [`docs/manifesto.txt`](/manifesto) — the canonical philosophical source for CAW, which predates this implementation. It is preserved exactly as written (including its original spelling and formatting). The protocol described in this white paper is the implementation of the specification it lays out.
