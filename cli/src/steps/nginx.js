 ...
 */
function ensureRateLimitZoneConf() {
...
function renderNginxConf({ domain, apiPort, frontendDist, uploadsDir, tls, nginxSupportsHttp2Directive }) {
  ...
  const sharedLocations = `
    ...
    location /api/ {
        # Early L7 defense — burst with nodelay lets normal traffic
        # (multiple requests fired near-simultaneously by a single page
        # load) through without queuing delay, while capping sustained
        # abuse before it reaches the Node event loop. Complements, not
        # replaces, the per-route Node-side limiters in server.ts (which
        # know about auth state and can return route-specific error
        # bodies — nginx here just returns a bare 503).
        #
