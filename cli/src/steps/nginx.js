 ...
 */
/**
 * Write the shared limit_req_zone definition to conf.d/ once. This must
 * live in the http {} context (nginx.conf includes conf.d/*.conf), not
 * inside a server {} block — sites-available/<domain> is server-context
 * only. Idempotent: if the file already exists (from a prior install on
 * this box — CAW makes it possible to run multiple domains on one VPS,
 * and this file would be shared between them), leave it alone rather
 * than overwriting, since a second install's zone size/rate shouldn't
 * silently change what the first install already relies on.
 */
export function ensureRateLimitZoneConf() {
  const confPath = '/etc/nginx/conf.d/caw-ratelimit.conf'
  // nginx.conf's http {} block only allows one limit_req_zone per zone
  // name, so if an operator runs more than one CAW domain on this host,
  // this file is shared between them. Default behavior is deliberately
  // conservative: leave an existing file alone, so installing/updating a
  // second domain on this box can never silently change the rate a first
  // domain is already depending on.
  //
  // CAW_NGINX_RATE_LIMIT_FORCE=1 is the intentional escape hatch — an
  // extra, explicit step rather than folding rate= into the normal .env +
  // `caw update` cycle (like burst= below). The friction is the point: if
  // more than one domain is on this host, a rate change here would apply
  // to all of them at once, not just the one the operator is currently
  // updating, so it shouldn't be something that happens as a side effect
  // of an unrelated single-domain update.
  const force = process.env.CAW_NGINX_RATE_LIMIT_FORCE === '1'
  if (fs.existsSync(confPath) && !force) {
    // Idempotent skip only counts as "already set up" if the file we're
    // looking at is actually one of ours — checked by the same marker
    // string patchMainNginxConfig() uses to decide a sites-available
    // file is caw-managed. Without this, a truncated write (interrupted
    // install) or a file that exists but never defines caw_general
    // (partial hand-edit) would be silently treated as done, and
    // nginx -t would fail downstream with no indication this file was
    // the cause — the error names the location block, not this zone
    // conf.
    const existing = fs.readFileSync(confPath, 'utf8')
    if (existing.includes('Auto-generated by caw cli') && existing.includes('caw_general')) {
      return confPath
    }
    console.log(warn(`  ${confPath} exists but doesn't look caw-managed (or is incomplete) — regenerating.`))
  }
  const rate = process.env.CAW_NGINX_RATE_LIMIT_PER_SEC || '50'
  const conf = `# Auto-generated by caw cli. Applies to every CAW domain on this host
# that shares this nginx instance.
# Not validated against production load — see PROJECT_BACKLOG.md's
# DDoS protection section for context.
#
# Unlike the Node-side limiters (ACTIONS_RATE_LIMIT_PER_MIN etc. in .env),
# this file is written once and left alone on later installs/updates by
# default, since a change here isn't scoped to a single domain. To change
# the rate on an existing host: set CAW_NGINX_RATE_LIMIT_PER_SEC and
# CAW_NGINX_RATE_LIMIT_FORCE=1 for one 'caw update' run, or edit this file
# directly and reload nginx.
limit_req_zone $binary_remote_addr zone=caw_general:10m rate=${rate}r/s;
...
/**
 * Write the shared limit_conn_zone definition to conf.d/ once. Same
 * rationale and same conf.d/ constraint as the request-rate zone above
 * (must live in http {}, not inside a server {} block) — but this one
 * caps concurrent connections rather than request rate, which is what
 * the DM WebSocket actually needs.
 *
 * Replaces an earlier attempt at this same goal that lived entirely in
 * websocket.ts (a hand-rolled Map<ip, count>, checked and incremented in
 * the Socket.IO auth middleware). Review of that PR found it had the same
 * class of gap client-side rate limiting always has: it counted only
 * after successful auth, so an unauthenticated flood never engaged the
 * cap, and the source IP it counted against was whatever value the
 * client's own X-Forwarded-For header claimed unless nginx was
 * independently rewriting it — which this location wasn't.
 * ngx_http_limit_conn_module counts before the app ever sees the
 * request, keyed on the real TCP peer address nginx itself observed
 * (binary_remote_addr, not any header), and updates its counter
 * synchronously in the connection-accept path rather than across an
 * async auth check the client's timing could race. Moving the cap here
 * closes the gap by moving it out of reach.
 */
function ensureConnLimitZoneConf() {
  const confPath = '/etc/nginx/conf.d/caw-connlimit.conf'
  const force = process.env.CAW_NGINX_CONN_LIMIT_FORCE === '1'
  if (fs.existsSync(confPath) && !force) return confPath
  const conf = `# Auto-generated by caw cli. Applies to every CAW domain on this host
# that shares this nginx instance.
# Caps concurrent connections per client IP — currently used to bound
# DM WebSocket connections (see location /socket.io/). Not scoped to a
# single domain, so — same as caw-ratelimit.conf — this is written once
# and left alone on later installs/updates by default. To change on an
# existing host: set CAW_NGINX_CONN_LIMIT_FORCE=1 for one 'caw update'
# run, or edit this file directly and reload nginx.
limit_conn_zone $binary_remote_addr zone=caw_conn:10m;
`
  fs.writeFileSync(confPath, conf)
  return confPath
}
...
export async function configureNginx(config, installDir) {
  ...
  ensureConnLimitZoneConf()

  const spinner = ora('Writing nginx server block...').start()
  try {
    // Inside the try now: this used to sit above the block, so an
    // EACCES or a full disk here surfaced as a raw, unhandled stack
    // trace instead of the same caught-and-reported path the
    // sitesAvailable write below gets.
    // Not fatal to the rest of the install — the rate limit zone is a
    // defense-in-depth layer, not something the server block depends
    // on to function — so this warns and continues rather than
    // aborting the whole nginx step over it.
    try {
      ensureRateLimitZoneConf()
    } catch (e) {
      console.log(warn(`  Could not write the rate-limit zone conf: ${e.message}`))
    }
...
export function patchMainNginxConfig(installDir) {
  ...
  // Ensure /etc/nginx/conf.d/caw-ratelimit.conf exists before applying patches
  try {
    ensureRateLimitZoneConf()
  } catch (e) {
    console.log(warn(`  Could not ensure caw-ratelimit.conf: ${e.message}`))
  }
...
function renderNginxConf({ domain, apiPort, frontendDist, uploadsDir, tls, nginxSupportsHttp2Directive }) {
  ...
  const sharedLocations = `
    ...
    location /api/ {
        # burst with nodelay lets normal traffic (multiple requests
        # fired near-simultaneously by a single page load) through
        # without queuing delay. Complements, not replaces, the
        # per-route Node-side limiters in server.ts (which know about
        # auth state and can return route-specific error bodies —
        # nginx here just returns a bare 429, see caw-ratelimit.conf).
        #
        # Correction: earlier versions of this comment (and the PR
        # description) framed this as "floods die at nginx before
        # reaching the app." That's not accurate for routes that
        # already have a Node-side limiter — /api/actions is 60/min
        # (~1r/s, #43) and /api/dm/relay is 1000/min (~16.7r/s), both
        # tighter than this zone's default 50r/s, so a flood aimed at
        # either dies in Express first, not here. What this actually
        # adds: a per-IP cap on /api/ in aggregate (which no per-route
        # limiter provides), and coverage for the many /api/* routes
        # that have no Node-side limiter at all.
        #
        # Unlike the zone's rate= (fixed at first install, see
        # ensureRateLimitZoneConf), burst IS re-read from env on every
        # regeneration (install/update), same as the Node-side limiters.
    ...
    location /socket.io/ {
        # X-Forwarded-For: this location previously appended nothing, so
        # getClientIp() in websocket.ts fell back to socket.handshake.address
        # (this proxy's loopback address, identical for every connection) —
        # collapsing the per-IP connection cap into a single node-wide one,
        # and admitting an unauthenticated flood freely since a client-set
        # header passed through unmodified. $proxy_add_x_forwarded_for
        # appends (not overwrites), matching the /api/ pattern that
        # server.ts's app.set('trust proxy', 'loopback') already trusts —
        # the real client ends up as the rightmost entry, same resolution
        # Express performs for req.ip. (Found in review of #45 by tentencaw.)
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        # Concurrent-connection cap, keyed on the real TCP peer nginx
        # itself observed (not a header) — see ensureConnLimitZoneConf()
        # for why this replaced an app-level attempt at the same goal.
        # Default 10 concurrent per IP, tune via
        # CAW_NGINX_WS_MAX_CONN if that's too tight for shared-NAT users.
        # Deliberately not named WS_MAX_CONNECTIONS_PER_IP — that name
        # belonged to the app-level Map this replaced (see websocket.ts
        # history), and reusing it risks an operator setting the old
        # variable and finding it silently does nothing now.
        limit_conn caw_conn ${process.env.CAW_NGINX_WS_MAX_CONN || 10};
        proxy_pass http://127.0.0.1:${apiPort};
        proxy_http_version 1.1;
