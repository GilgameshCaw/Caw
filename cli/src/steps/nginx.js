 ...
 * rationale and same conf.d/ constraint as a request-rate zone would have
 ...
 * the Socket.IO auth middleware). Review of that PR (#45, tentencaw)
 * found it had the same class of gap client-side rate limiting always
 * has: it counted only after successful auth, so an unauthenticated
 * flood never engaged the cap, and the source IP it counted against was
 * whatever value the client's own X-Forwarded-For header claimed unless
 * nginx was independently rewriting it — which this location wasn't.
