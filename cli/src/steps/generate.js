...
function buildEnvVars(nodeType, config) {
  ...
  // Canonical public origin for the install — used for URLs the API hands back
  // to the browser (uploaded image URLs, og tags, short links) and the CawAI
  // citation links. Without this, upload routes fall back to
  // http://localhost:4000, which a browser served over HTTPS rejects as mixed
  // content. publicUrl() in client/src/api/util/publicUrl.ts reads HOST_DOMAIN
  // (then SHORTURL_DOMAIN as back-compat fallback). SHORTURL_DOMAIN is no longer
  // written here by default — set it manually only for an external URL shortener
  // on a different host, where it overrides HOST_DOMAIN for short links.
  if (config.domain) env.HOST_DOMAIN = `https://${config.domain}`
