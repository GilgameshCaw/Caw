...
export function getWriteConfig(): WriteConfig | null {
  ...
  const client = process.env.CAW_NETWORK_ID
...
export async function handleToolCall(
  ...
  try {
    ...
    if (WRITE_TOOLS.some(t => t.name === name)) {
      if (!writeConfig) {
        return {
          content: [{ type: "text", text: "Write tools require CAW_SESSION_KEY, CAW_SENDER_ID, CAW_NETWORK_ID, CAW_CHAIN_ID, and CAW_VERIFYING_CONTRACT environment variables." }],
          isError: true,
        }
      }
