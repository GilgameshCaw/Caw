...
/// @title ICawCapOracle
/// @notice Minimal interface for the L2 cap oracle. CawActions reads only
///         `capForAction`; the full CawCapOracle ABI lives in CawCapOracle.sol.
interface ICawCapOracle {
  /// @notice Returns the per-action CAW cap given the manifesto baseline and
  ///         the ETH-denominated ceiling. If the oracle is stale or unpopulated,
  ///         returns `baseline` (cap dormant). Otherwise returns
  ///         `min(baseline, ethCap / twap)`.
