...
circuit, instead of running ecrecover per action. Constant-cost verifier
(~265K gas, canonical SP1Verifier on Base Sepolia) replaces the per-action
ecrecover; **break-even is ~57 actions per batch**. Cawonce conflicts in the
ZK path skip the affected slots rather than reverting the whole batch.
